package Bot::BasicBot::Pluggable::Module::Brownian::TwitterWatch;
...
sub said {
    ...
    if ( my ( $command, $params ) = $mess->{body} =~ /^!(twitter\w+)\s*(.*)/i )
    {
        ...
        if ( lc $command eq 'twitterwatch' ) {
            ...
            $message = "Currently watching for: " . join ',',
              map { qq["$_"] } keys %$chansearches;
        }
...
sub tick {
    ...
    for my $channel ( keys %$searches ) {
        ...
        for my $searchterm ( keys %{ $searches->{$channel} } ) {
            ...
            warn
              __PACKAGE__.": Searching for '$searchterm' after $last_id on behalf of $channel"
            ...
            if ($last_id) {
                my %tweets_from_user;
                for my $result ( grep { $_->{id} > $last_id }
                    @{ $results->{results} } )
                {
                    if ( $ignore->{ lc $result->{from_user} } ) {
                        warn __PACKAGE__.": Ignoring tweet from $result->{from_user} :"
                          . $result->{text}
                          if ( $self->get('user_debug') == 1 );
                        return;
                    }
                    ...
                    if ( $user_details->{statuses_count} < 40 ) {
                        warn __PACKAGE__.": Ignoring new spam account $result->{from_user}"
                          if ( $self->get('user_debug') == 1 );
                        next;
                    }
...
Further hacked on by Nuno Nunes, C<< <nuno at nunonunes dot org> >>
